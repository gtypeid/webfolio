...
function TestFunction()
{
    ...
    if(gVTest)
    {
        let WorkBox = document.getElementsByClassName("work-box")[0];
...
function AppendSlateAndTree(targetClassName, newSettingParam)
{
    ...
    if( gSState.ClickSlate)
    {
        ...
        $("#" + MakeTreeID).append("<div class=tree"+" id=" + MakeId +">" +ChildBlank+ TargetClassName + "</div>");
    ...
    else
    {
        ...
        MakeId = gSNaming.TreeObjId + MakeIndex; // ObjCount(gSNaming.TreeObjId);
        CreateObj(gSNaming.TreeObjId, MakeId, MakeIndex);
        $(".tree-box").append("<div class=tree"+" id=" + MakeId +">" +ChildBlank+ TargetClassName + "</div>");
    }
...
function ClickTree(event)
{
    ...
    if(event.button == 0)
    {
        if( IsEmpty(gSState.ClickTree) )
        {
            ...
            if(TreeObj)
            {
                ReMoveClass_ID(gSState.ClickTree, "tree-select");
        ...
        if(TreeObj)
        {
            AddClass_ID( gSState.ClickTree, "tree-select");
...
function SlateSelect(innerObjId , bSelect)
{
    ...
    if(SlateObj)
    {
        if(bSelect) 
        {
            AddClass_ID(innerObjId, "slate-select");
        }
        else
        {
            ReMoveClass_ID(innerObjId, " slate-select");
        }
    }
}

function MouseDown(buttonType, event)
{
    if(buttonType == 1)
    {
...
function MenuClick(self, param)
{
    ...
    if(param == 0)
    {
        if(CurrentObj && CurrentObj_Tree)
        {
            ...
            if(!bObj)
            {
                console.log("string DELETE " + CurrnetId);
                ...
                CssDeleteRule(gSlateMap.get(CurrnetId).IDNam, gSlateMap.get(CurrnetId).Index);
            ...
            else
            {
                ...
                if(OneChild)
                {
                    for(var i = Childs.length - 1; i >= 0; --i )
                    {
                        ...
                        CssDeleteRule(gSlateMap.get(Childs[i]).IDNam, gSlateMap.get(Childs[i]).Index);
                ...
                else
                {
                    ...
                    for(var i = DepthArray.length - 1; i >= 0; --i )
                    {
                        ...
                        CssDeleteRule(gSlateMap.get(CurrentTarget).IDNam, gSlateMap.get(CurrentTarget).Index);
...
{
    let UtilBox =  document.getElementsByClassName("mouse-util-box")[0];
    if(UtilBox)
    {
        if(bFlag)
        {
...
function CopyButtonClick(target, param)
{
    ...
    let WorkdString = document.getElementsByClassName("work-box")[0].innerHTML;
    let MakeString = 
    "<!DOCTYPE html>" +
    "<html lang=en>" + 
    "<head>" +
